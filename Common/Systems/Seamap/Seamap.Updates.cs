...
public partial class Seamap
{
    ...
    public static void PlaceRockCluster(Vector2 center) {
        var val = Main.rand.NextFloat(6.28f);

        for (var i = 0; i < 3; i++) {
            var val2 = val + i * 6.28f / 3f;
    ...
    public static void SpawnSeagullFlock(int amount) {
        var Pos = new Vector2(Main.rand.Next(Main.screenWidth) + Main.screenPosition.X, Main.screenPosition.Y - 100);
        var PosBuffer = new List<Vector2>();
        for (var i = 0; i < amount; i++) {
            ...
            var boidCheck = 0;
            for (var j = 0; j < PosBuffer.Count; j++) {
